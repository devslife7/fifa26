...
function CompactBracketView({ groupPredictions, knockoutPredictions, thirdPlaceTiebreaker }: Props) {
  ...
  return (
    <div className="flex gap-6 p-8 bg-slate-50 w-full h-[800px] items-center justify-center">
      {rounds.map(round => {
          ...
          <div key={round} className="flex flex-col justify-around h-full flex-1 gap-4">
            ...
            <div className="flex flex-col justify-around h-full">
              {matches.map(match => {
                ...
                return (
                  <div key={match.id} className="flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden text-sm">
                    <div className={`flex items-center gap-3 px-4 py-3 border-b border-slate-100 ${isHomeWinner ? 'bg-slate-50 font-bold text-slate-900' : 'text-slate-500'}`}>
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3 ${isAwayWinner ? 'bg-slate-50 font-bold text-slate-900' : 'text-slate-500'}`}>
  ...
  const handleScreenshot = async () => {
    ...
    try {
      const canvas = await html2canvas(captureRef.current, {
        backgroundColor: '#f8fafc', // slate-50 to match typical background
        scale: 2, // higher res
        logging: false,
        useCORS: true,
        windowWidth: 1200,
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <div className="overflow-hidden h-0 w-0 absolute left-[-9999px]">
        <div ref={captureRef} className="bg-slate-50 p-12 flex flex-col items-center w-[1200px]">
          <div className="flex items-center justify-center gap-6 mb-10">
            <div className="text-7xl">🏆</div>
            <div>
              <h2 className="text-4xl font-black text-slate-900">My FIFA 26 Predictions</h2>
              <p className="text-slate-500 font-medium text-xl mt-1">Champion: <span className="text-primary font-bold">{champion.name}</span></p>
            </div>
          </div>
          ...
          <div className="mt-10 text-slate-400 font-bold tracking-widest uppercase text-base">
            fifa26.app
          </div>
        </div>
      </div>

      <div className="flex flex-col items-center justify-center w-full p-8 rounded-3xl bg-slate-50">
      ...
      <div className="mt-8 w-full max-w-sm">
        <div className="flex gap-3 mb-6">
          <button
            ...
            className="flex-1 py-3 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
          >
