    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        <div className="bg-white px-6 py-4 border-b border-slate-200 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-xl font-black text-slate-900">{prediction.display_name}&apos;s Predictions</h2>
            {championTeam && (
              <div className="flex items-center gap-1.5 mt-1 text-sm font-semibold text-slate-500">
                <span className="text-xs font-medium text-slate-400">Predicted Champion:</span>
        ...
        {/* Tabs */}
        <div className="bg-white px-6 border-b border-slate-200 flex-shrink-0 flex gap-6">
          <button
            onClick={() => setActiveTab('groups')}
            className={`py-3 text-sm font-bold border-b-2 transition-colors ${
              activeTab === 'groups' ? 'border-primary text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'
            }`}
          >
            Group Stage
          </button>
          <button
            ...
            className={`py-3 text-sm font-bold border-b-2 transition-colors ${
              activeTab === 'bracket' ? 'border-primary text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'
            }`}
          >
            Knockout Bracket
          </button>
